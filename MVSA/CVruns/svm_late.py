...
class custom_transform(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        return [X_text, img_df]    

# need to wrap around SVC so that we can automatically calculate gamma
class custom_learner(BaseEstimator, TransformerMixin):
    def __init__(self, C1=1, C2=1, gmma=None):
        self.C1 = C1
        self.C2 = C2
        self.gmma = gmma
        self.est1 = SVC(C=C1, kernel='linear', probability=True)
        self.est2 = SVC(C=C2, kernel='rbf', gamma=gmma, probability=True)
        
    def fit(self, X, y=None):
        if self.gmma == None:
            self.gmma = 1/(2*np.mean(metrics.pairwise_distances(X[1]))**2)
            self.est2 = SVC(C=self.C2, kernel='rbf', gamma=self.gmma, 
                            probability=True)
        self.est1.fit(X[0], y)
        self.est2.fit(X[1], y)
        return self
    ...
    def predict(self, X, y=None):
        p1 = self.est1.predict_proba(X[0])[:,1]
        p2 = self.est2.predict_proba(X[1])[:,1]
        return(0.5*(p1+p2))
...
params = {
        'clf__C1': np.geomspace(1e-5, 1e3, 20),
        'clf__C2': np.geomspace(1e-5, 1e3, 20)
        }
...
if __name__ == "__main__":
    ...
    print('Late Fusion best score: {}'.format(grid.best_score_))
